    ...
    , displayUpdatePending_(false)
{
  ...
  display_.fill(0);
  keys_.fill(0);
  memcpy((char*)&mem_[CHIP8_defs::FONTSET_START_ADDRESS],
         &CHIP8_defs::fontset,
         CHIP8_defs::FONTSET_SIZE);
  srand(time(NULL));
...
void CHIP8_impl::nibble_6()
{
  ...
  uint8_t Vx = (opcode_ & 0x0F00u) >> 8u;
  uint8_t kk = (opcode_ & 0x00FFu);
  // std::cout << "Set reg[" << std::hex << (int)Vx << "] = " << (int)kk <<
  // std::endl;
  reg_[Vx] = kk;
}
...
void CHIP8_impl::nibble_A()
{
  ...
  // std::cout << "The value of register I is set to:" << std::hex << val <<
  // std::endl;
  index_ = val;
}
...
{
  // std::cout << " Display n-byte sprite starting at memory location I at (Vx,
  // Vy), set VF = collision." << std::endl;
  ...
  for (size_t i = 0; i < height; i++) {
    ...
    //   std::cout << "row bits as byte: " << std::hex << (int)rowBitsAsByte <<
    //   std::endl;

    for (size_t j = 0; j < 8; j++) {
      size_t pixelId = 8 - j - 1;
      uint8_t pixelToSet = ((rowBitsAsByte & (0x01 << pixelId)) >> pixelId);
      //  std::cout << "Column: " << j << " pixel: " << (int)pixelToSet<<
      //  std::endl;
      uint8_t pixelOnScreen =
          display_[displayIdx0 + j + i * CHIP8_defs::DISPLAY_WIDTH];
      collision = collision || ((pixelToSet & 0x01) & (pixelOnScreen & 0x01));
      display_[displayIdx0 + j + i * CHIP8_defs::DISPLAY_WIDTH] =
          pixelToSet ^ pixelOnScreen;
    }
...
{
  // std::cout << "Opcode: " << std::showbase << std::hex << (int)opcode_ <<
  // std::endl;
