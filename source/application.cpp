...
bool Application::init(std::unique_ptr<IEmulator> emulator)
{
  ...
  auto calculatePixelSizeForRectCount = [=](int rectCount) {
    return this->rectWidth_ * rectCount + (2 * rectCount + 1) * this->margin_;
  };
  ...
  window_ = SDL_CreateWindow("CHIP8 EMULATOR",
  ...
  if (SDL_SetRenderDrawColor(renderer_, 0xff, 0xff, 0xff, 0xff) < 0) {
    std::cerr << "Failed to set renderer drawing color: " << SDL_GetError()
              << std::endl;
    return false;
  }
